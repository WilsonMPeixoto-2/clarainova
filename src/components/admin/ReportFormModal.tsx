...
import { ReportTagSelector, type ReportTag } from "./ReportTagSelector";
import { ReportTemplates, type ReportTemplate } from "./ReportTemplates";
...
export function ReportFormModal({
  ...
  useEffect(() => {
    if (open) {
      if (report) {
        ...
        setSelectedTagIds(report.tags?.map((t) => t.id) || []);
      } else {
        setTitle("");
        setContent("");
        setSelectedTagIds([]);
  ...
  const handleSave = async () => {
    ...
    await onSave(title.trim(), content.trim(), selectedTagIds);
  };
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 py-4">
            {/* Template button - only for new reports */}
            {!isEditing && (
              <Button
                type="button"
                variant="outline"
